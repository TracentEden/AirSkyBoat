...
namespace itemutils
{
            ...
            while (_sql->NextRow() == SQL_SUCCESS)
            {
                ...
                if (PItem != nullptr)
                {
                    ...
                    if (PItem->isType(ITEM_EQUIPMENT))
                    {
                        ...
                        ((CItemEquipment*)PItem)->setRace(_sql->GetUIntData(24));
    ...
    void LoadDropList()
    {
        int32 ret = _sql->Query("SELECT dropId, itemId, dropType, itemRate, groupId, groupRate FROM mob_droplist WHERE dropid < %u ORDER BY dropId, dropType, groupId;", MAX_DROPID);
