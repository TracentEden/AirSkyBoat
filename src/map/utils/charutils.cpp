...
namespace charutils
{
    ...
    CCharEntity* LoadChar(uint32 charId)
    {
        ...
        if (rset && rset->rowsCount() && rset->next())
        {
            ...
            PChar->profile.fame[15]     = (uint16)_sql->GetIntData(19); // Special Events
                ...
                case SLOT_RANGED:
                {
                    if (PItem->isType(ITEM_WEAPON))
                    {
                        ...
                        if (weapon)
                        {
                            ...
                            else if (static_cast<CItemWeapon*>(PItem)->getSkillType() == weapon->getSkillType() ||
                                     static_cast<CItemWeapon*>(PItem)->getSubSkillType() == weapon->getSubSkillType())
                            {
            ...
            if (PChar->GetMLevel() >= PAbility->getLevel())
            {
                if (PAbility->getID() < ABILITY_HEALING_RUBY && PAbility->getID() != ABILITY_PET_COMMANDS && CheckAbilityAddtype(PChar, PAbility))
                {
                    ...
                    if (charge)
                    {
                        ...
                        // This division is era only and not needed in LSB
    ...
    void TrySkillUP(CCharEntity* PChar, SKILLTYPE SkillID, uint8 lvl, bool forceSkillUp, bool useSubSkill)
    {
        ...
        if (((PChar->WorkingSkills.rank[rawSkillID] != 0) && !(PChar->WorkingSkills.skill[rawSkillID] & 0x8000)) || useSubSkill)
        {
            ...
            // https://github.com/HorizonFFXI/HorizonXI/pull/487
            // skill up hack
    ...
    */
    ...
    void ReadFishingHistory(CCharEntity* PChar)
    {
        ...
        if (ret != SQL_ERROR && _sql->NumRows() != 0 && _sql->NextRow() == SQL_SUCCESS)
        {
            ...
            _sql->GetData(0, &fishList, &length);
            ...
            PChar->m_fishHistory.fishLinesCast  = (uint32)_sql->GetIntData(1);
            PChar->m_fishHistory.fishReeled     = (uint32)_sql->GetIntData(2);
            PChar->m_fishHistory.fishLongest    = (uint32)_sql->GetIntData(3);
            PChar->m_fishHistory.fishLongestId  = (uint32)_sql->GetIntData(4);
            PChar->m_fishHistory.fishHeaviest   = (uint32)_sql->GetIntData(5);
            PChar->m_fishHistory.fishHeaviestId = (uint32)_sql->GetIntData(6);
        }
    }
    ...
    void WriteFishingHistory(CCharEntity* PChar)
    {
        ...
        _sql->EscapeStringLen(fishList, (const char*)&PChar->m_fishHistory.fishList, sizeof(PChar->m_fishHistory.fishList));
