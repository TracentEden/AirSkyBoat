...
bool CLuaBaseEntity::pathThrough(sol::table const& pointsTable, sol::object const& flagsObj)
{
    ...
    else
    {
        ...
        {
            points.emplace_back({ { (float)pointsTable[i], (float)pointsTable[i + 1], (float)pointsTable[i + 2], 0, 0 }, 0 });
        }
    }
...
void CLuaBaseEntity::resetPlayer(const char* charName)
{
    ...
    {
        id   = _sql->GetUIntData(0);
        zone = _sql->GetUIntData(12);
        x    = _sql->GetFloatData(14);
        y    = _sql->GetFloatData(15);
        z    = _sql->GetFloatData(16);
    }
...
uint32 CLuaBaseEntity::getWorldPassRedeemTime()
{
    ...
    uint64 ret       = _sql->Query(wpQuery, m_PBaseEntity->id);
    uint64 rafTime   = 0;

    if (ret != SQL_ERROR && _sql->NumRows() != 0)
    {
        _sql->NextRow();
        rafTime = _sql->GetUInt64Data(0);
...
uint32 CLuaBaseEntity::getWorldpassId(uint32 targid)
{
    ...
    uint32 ret = _sql->Query(wpQuery, m_PBaseEntity->id, targid);
...
auto CLuaBaseEntity::getAvailableContestRewards(uint16 contestId) -> sol::table
{
    if (m_PBaseEntity && m_PBaseEntity->objtype == TYPE_PC)
    {
        ...
        int32 ret = _sql->Query(Query.c_str(), m_PBaseEntity->name, contestId);
        ...
        if (ret != SQL_ERROR && _sql->NumRows() > 0)
        {
            ...
            while (_sql->NextRow() == SQL_SUCCESS)
            {
                ...
                reward["id"]    = _sql->GetUIntData(0);
                reward["rank"]  = _sql->GetUIntData(1);
                reward["share"] = _sql->GetUIntData(2);
...
auto CLuaBaseEntity::getAwardHistory() -> sol::table
{
    if (m_PBaseEntity->objtype == TYPE_PC)
    {
        ...
        int32 ret = _sql->Query(Query, m_PBaseEntity->name.c_str());
        if (!(ret == SQL_ERROR) && _sql->NumRows() > 0)
        {
            while (_sql->NextRow() == SQL_SUCCESS)
            {
                table[sql->GetUIntData(0)] = _sql->GetUIntData(1);
            }
        }
        else
        {
